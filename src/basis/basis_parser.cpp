...
Basis_Parser::Basis_Parser(const std::string& basis_name_in, bool is_spherical_in,  MPI_info &mpi_info, Molecule &molecule) :
...
void Basis_Parser::build_atomic_orbitals(const Molecule& molecule) {
  uint contraction= 0;
  SHELL::Shell_Type shell_type = SHELL::NA;
...
void Basis_Parser::broadcast(MPI_info& mpi_info) {
  MPI_info::barrier();
