...
void Basis::gpu_alloc(int mc_pair_num, Molec& molec) {
	cudaError_t_Assert(cudaMalloc((void**) &d_basis.am, sizeof(int) * qc_nshl), __FILE__, __LINE__);
	cudaError_t_Assert(cudaMalloc((void**) &d_basis.at, sizeof(int) * qc_nshl), __FILE__, __LINE__);
	cudaError_t_Assert(cudaMalloc((void**) &d_basis.isgs, sizeof(int) * (qc_nshl+1)), __FILE__, __LINE__);
	cudaError_t_Assert(cudaMalloc((void**) &d_basis.stop_list, sizeof(int) * (qc_nshl+1)), __FILE__, __LINE__);
	cudaError_t_Assert(cudaMalloc((void**) &d_basis.alpha, sizeof(double) * qc_nprm), __FILE__, __LINE__);
	cudaError_t_Assert(cudaMalloc((void**) &d_basis.norm, sizeof(double) * qc_nprm), __FILE__, __LINE__);
	cudaError_t_Assert(cudaMalloc((void**) &d_basis.nw_co, sizeof(double) * nw_nbf * nw_nmo[0]), __FILE__, __LINE__);
	cudaError_t_Assert(cudaMalloc((void**) &d_basis.icgs, sizeof(double) * nw_nbf * mc_pair_num * 2), __FILE__, __LINE__);

	cudaError_t_Assert(cudaMalloc((void**) &d_basis.pos, sizeof(double) * mc_pair_num * 6), __FILE__, __LINE__);
	cudaError_t_Assert(cudaMalloc((void**) &d_basis.apos, sizeof(double) * molec.natom * 3), __FILE__, __LINE__);

	cudaError_t_Assert(cudaMemcpy(d_basis.am, h_basis.am, sizeof(int) * qc_nshl, cudaMemcpyHostToDevice), __FILE__, __LINE__);
	cudaError_t_Assert(cudaMemcpy(d_basis.at, h_basis.at, sizeof(int) * qc_nshl, cudaMemcpyHostToDevice), __FILE__, __LINE__);
	cudaError_t_Assert(cudaMemcpy(d_basis.isgs, h_basis.isgs, sizeof(int) * (qc_nshl+1), cudaMemcpyHostToDevice), __FILE__, __LINE__);
	cudaError_t_Assert(cudaMemcpy(d_basis.stop_list, h_basis.stop_list, sizeof(int) * (qc_nshl+1), cudaMemcpyHostToDevice), __FILE__, __LINE__);
	cudaError_t_Assert(cudaMemcpy(d_basis.alpha, h_basis.alpha, sizeof(double) * qc_nprm, cudaMemcpyHostToDevice), __FILE__, __LINE__);
	cudaError_t_Assert(cudaMemcpy(d_basis.norm, h_basis.norm, sizeof(double) * qc_nprm, cudaMemcpyHostToDevice), __FILE__, __LINE__);
	...
	double pos[molec.natom * 3];
	for(int i=0;i<molec.natom;i++) {
		for(int j=0;j<3;j++) {
			pos[i*3+j] = molec.atom[i].pos[j];
		}
	}
	cudaError_t_Assert(cudaMemcpy(d_basis.apos, pos, sizeof(double) * molec.natom * 3, cudaMemcpyHostToDevice), __FILE__, __LINE__);
}
void Basis::gpu_free() {
	cudaError_t_Assert(cudaFree(d_basis.am), __FILE__, __LINE__);
	cudaError_t_Assert(cudaFree(d_basis.at), __FILE__, __LINE__);
	cudaError_t_Assert(cudaFree(d_basis.isgs), __FILE__, __LINE__);
	cudaError_t_Assert(cudaFree(d_basis.stop_list), __FILE__, __LINE__);
	cudaError_t_Assert(cudaFree(d_basis.alpha), __FILE__, __LINE__);
	cudaError_t_Assert(cudaFree(d_basis.norm ), __FILE__, __LINE__);
	cudaError_t_Assert(cudaFree(d_basis.nw_co), __FILE__, __LINE__);
	cudaError_t_Assert(cudaFree(d_basis.icgs), __FILE__, __LINE__);

	cudaError_t_Assert(cudaFree(d_basis.pos), __FILE__, __LINE__);
	cudaError_t_Assert(cudaFree(d_basis.apos), __FILE__, __LINE__);
}
...
__global__ void device_cgs_get(int mc_pair_num, int qc_nshl, BasisData d_basis) {
	...
	if(tidy < qc_nshl && tidx < mc_pair_num) { //if tidy is less than number of shells
		...
		for(i=d_basis.stop_list[tidy]; i<d_basis.stop_list[tidy+1]; i++) {
			rad = rad + exp( -d_basis.alpha[i] * r2) * d_basis.norm[i];
		}

		if(iam == 0) {
			d_basis.icgs[(ic+0)*mc_pair_num+tidx] = rad;
		}
		else if(iam == -1) {
			d_basis.icgs[(ic+0)*mc_pair_num+tidx] = rad; 
			d_basis.icgs[(ic+1)*mc_pair_num+tidx] = rad*dr[0];
			d_basis.icgs[(ic+2)*mc_pair_num+tidx] = rad*dr[1];
			d_basis.icgs[(ic+3)*mc_pair_num+tidx] = rad*dr[2];
		}
		else if(iam == 1) {
			d_basis.icgs[(ic+0)*mc_pair_num+tidx] = rad*dr[0];
			d_basis.icgs[(ic+1)*mc_pair_num+tidx] = rad*dr[1];
			d_basis.icgs[(ic+2)*mc_pair_num+tidx] = rad*dr[2];
		}
		else if(iam == 2) {
			...
			d_basis.icgs[(ic+0)*mc_pair_num+tidx] = rad*ang[0];
			d_basis.icgs[(ic+1)*mc_pair_num+tidx] = rad*ang[1];
			d_basis.icgs[(ic+2)*mc_pair_num+tidx] = rad*ang[2];
			d_basis.icgs[(ic+3)*mc_pair_num+tidx] = rad*ang[3];
			d_basis.icgs[(ic+4)*mc_pair_num+tidx] = rad*ang[4];
		}
		else if (iam == 3) {
			...
			d_basis.icgs[(ic+3)*mc_pair_num+tidx] = rad*ang[3];
			d_basis.icgs[(ic+4)*mc_pair_num+tidx] = rad*ang[4];
			d_basis.icgs[(ic+5)*mc_pair_num+tidx] = rad*ang[5];
			d_basis.icgs[(ic+6)*mc_pair_num+tidx] = rad*ang[6];
		}
		else if (iam == 4) {
			...
			d_basis.icgs[(ic+5)*mc_pair_num+tidx] = rad*ang[5];
			d_basis.icgs[(ic+6)*mc_pair_num+tidx] = rad*ang[6];
			d_basis.icgs[(ic+7)*mc_pair_num+tidx] = rad*ang[7];
			d_basis.icgs[(ic+8)*mc_pair_num+tidx] = rad*ang[8];
		}
	}
}
...
void Basis::device_psi_get(double *occ1,
	...
	cublasStatusAssert(cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N,
			...
			&d_basis.icgs[0], mc_pair_num * 2,
			&d_basis.nw_co[iocc1*nw_nbf], nw_nbf, 
			&beta, occ1, mc_pair_num), __FILE__, __LINE__);
	cublasStatusAssert(cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N,
		       	mc_pair_num, iocc2-iocc1, nw_nbf, &alpha, 
			&d_basis.icgs[mc_pair_num], mc_pair_num * 2,
			&d_basis.nw_co[iocc1*nw_nbf], nw_nbf, 
			&beta, occ2, mc_pair_num), __FILE__, __LINE__);
	cublasStatusAssert(cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N,
		       	mc_pair_num, ivir2-ivir1, nw_nbf, &alpha, 
			&d_basis.icgs[0], mc_pair_num * 2,
			&d_basis.nw_co[ivir1*nw_nbf], nw_nbf, 
			&beta, vir1, mc_pair_num), __FILE__, __LINE__);
	cublasStatusAssert(cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N,
		       	mc_pair_num, ivir2-ivir1, nw_nbf, &alpha, 
			&d_basis.icgs[mc_pair_num], mc_pair_num * 2,
			&d_basis.nw_co[ivir1*nw_nbf], nw_nbf, 
			&beta, vir2, mc_pair_num), __FILE__, __LINE__);

	cublasStatusAssert(cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N,
		       	mc_pair_num, ivir2-iocc1, nw_nbf, &alpha, 
			&d_basis.icgs[0], mc_pair_num * 2,
			&d_basis.nw_co[iocc1*nw_nbf], nw_nbf, 
			&beta, psi1, mc_pair_num), __FILE__, __LINE__);
	cublasStatusAssert(cublasDgemm(handle, CUBLAS_OP_N, CUBLAS_OP_N,
		       	mc_pair_num, ivir2-iocc1, nw_nbf, &alpha, 
			&d_basis.icgs[mc_pair_num], mc_pair_num * 2,
			&d_basis.nw_co[iocc1*nw_nbf], nw_nbf, 
			&beta, psi2, mc_pair_num), __FILE__, __LINE__);
