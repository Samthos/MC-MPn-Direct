...
void GF2_F12_VBX::energy_f12(std::vector<std::vector<double>>& egf, std::unordered_map<int, Wavefunction_Type>& wavefunctions, Electron_Pair_List_Type* electron_pair_list, Electron_List_Type* electron_list) {
  ...
  std::transform(core_11p.begin(), core_11p.end(), core_11p.begin(), multiply_by_two);
  std::transform(core_12p.begin(), core_12p.end(), core_12p.begin(), multiply_by_two);
  std::transform(core_22p.begin(), core_22p.end(), core_22p.begin(), multiply_by_two);
  std::transform(core_13.begin() , core_13.end() , core_13.begin() , multiply_by_two);
  std::transform(core_23.begin() , core_23.end() , core_23.begin() , multiply_by_two);
...
void calculate_bx_t_fa_4e_help(
    std::vector<double>& T_ip_jo, const std::vector<double>& S_ip_jo_1, const std::vector<double>& S_ip_jo_2, 
    std::vector<double>& T_ip_io, const std::vector<double>& S_io_jo,
    std::vector<double>& out, const std::vector<double>& S_ip_io, const std::vector<double>& S_ip,
    const double alpha, size_t size, size_t  size_ep
...
double GF2_F12_VBX::calculate_bx_t_fa_4e(const Electron_Pair_List_Type* electron_pair_list, const Electron_List_Type* electron_list) {
  ...
  std::transform(correlation_factor->f12p_a.begin(), correlation_factor->f12p_a.end(), electron_pair_list->rv.begin(), T_ip.begin(), std::multiplies<>());
  for (int io = 0, idx = 0; io < electron_list->size(); ++io) {
    for(int jo = 0; jo < electron_list->size(); ++jo, ++idx) {
      T_io_jo[idx] = correlation_factor->f12o[idx] * electron_list->inverse_weight[io] * electron_list->inverse_weight[jo];
    }
  }
  calculate_bx_t_fa_4e_help(T_ip_jo, traces.p23, traces.k23, T_ip_io, T_io_jo, core_13, traces.k13, T_ip,  c_c3, electron_list->size(), electron_pair_list->size());
  calculate_bx_t_fa_4e_help(T_ip_jo, traces.p23, traces.v23, T_ip_io, T_io_jo, core_13, traces.v13, T_ip, -c_c3, electron_list->size(), electron_pair_list->size());
  calculate_bx_t_fa_4e_help(T_ip_jo, traces.p23, traces.k13, T_ip_io, T_io_jo, core_13, traces.k23, T_ip,  c_c4, electron_list->size(), electron_pair_list->size());
  calculate_bx_t_fa_4e_help(T_ip_jo, traces.p23, traces.v13, T_ip_io, T_io_jo, core_13, traces.v23, T_ip, -c_c4, electron_list->size(), electron_pair_list->size());
                                                                                                  
  calculate_bx_t_fa_4e_help(T_ip_jo, traces.p13, traces.k13, T_ip_io, T_io_jo, core_23, traces.k23, T_ip,  c_c3, electron_list->size(), electron_pair_list->size());
  calculate_bx_t_fa_4e_help(T_ip_jo, traces.p13, traces.v13, T_ip_io, T_io_jo, core_23, traces.v23, T_ip, -c_c3, electron_list->size(), electron_pair_list->size());
  calculate_bx_t_fa_4e_help(T_ip_jo, traces.p13, traces.k23, T_ip_io, T_io_jo, core_23, traces.k13, T_ip,  c_c4, electron_list->size(), electron_pair_list->size());
...
void calculate_bx_t_fb_3e_help(
    std::vector<double>& T_io_jo, const std::vector<double>& S_io_ko, const std::vector<double>& S_jo_ko,
    std::vector<double>& out, const std::vector<double>& S_io_jo,
    const double alpha, size_t size
    ) {
  cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans,
      size, size, size,
      1.0,
  ...
  for(int io = 0, idx=0; io < size; ++io) {
    for (int jo = 0; jo < size; ++jo, ++idx) {
      out[idx] += alpha * T_io_jo[idx] * S_io_jo[idx];
    }
  }
}
double GF2_F12_VBX::calculate_bx_t_fb_3e(const Electron_Pair_List_Type* electron_pair_list, const Electron_List_Type* electron_list) {
  ...
  for (int io = 0, idx=0; io < electron_list->size(); ++io) {
    for (int ko = 0; ko < electron_list->size(); ++ko, ++ idx) {
      T_io_ko[idx] = traces.op12[idx] * traces.ok12[idx] * electron_list->inverse_weight[io] * electron_list->inverse_weight[ko];
    }
  }
  for (int jo = 0; jo < electron_list->size(); ++jo) {
    double t_jo = 0.0;
    for (int io = 0; io < electron_list->size(); ++io) {
      ...
      for (int ko = 0; ko < electron_list->size(); ++ko) {
        t_io += correlation_factor->f12o[jo * traces.electrons + ko] * T_io_ko[io * traces.electrons + ko];
      }
      t_jo += t_io * correlation_factor->f12o_b[jo * traces.electrons + io];
    }
    core_11o[jo] += c_c3 * t_jo * electron_list->inverse_weight[jo];
  }

  for (int io = 0, idx=0; io < electron_list->size(); ++io) {
    for (int ko = 0; ko < electron_list->size(); ++ko, ++ idx) {
      T_io_ko[idx] = correlation_factor->f12o_b[idx] * electron_list->inverse_weight[io] * electron_list->inverse_weight[ko];
    }
  }
  for (int jo = 0, idx=0; jo < electron_list->size(); ++jo) {
    for (int ko = 0; ko < electron_list->size(); ++ko, ++ idx) {
      T_jo_ko[idx] = correlation_factor->f12o[idx] * traces.op11[ko] * electron_list->inverse_weight[jo];
    }
  }
  calculate_bx_t_fb_3e_help(
    T_io_jo, T_io_ko, T_jo_ko,
    core_12o, traces.ok12,
    c_c3, electron_list->size());

  std::transform(T_io_ko.begin(), T_io_ko.end(), traces.op12.begin(), T_io_ko.begin(), std::multiplies<>());
  for (int jo = 0, idx=0; jo < electron_list->size(); ++jo) {
    for (int ko = 0; ko < electron_list->size(); ++ko, ++ idx) {
      T_jo_ko[idx] = traces.ok12[idx] * electron_list->inverse_weight[jo];
    }
  }
  calculate_bx_t_fb_3e_help(
    T_io_jo, T_io_ko, T_jo_ko,
    core_12o, correlation_factor->f12o,
    c_c4, electron_list->size());
  
  for (int io = 0, idx=0; io < electron_list->size(); ++io) {
    for (int ko = 0; ko < electron_list->size(); ++ko, ++ idx) {
      T_io_ko[idx] = traces.ok12[idx] * electron_list->inverse_weight[io] * electron_list->inverse_weight[ko];
    }
  }
  for (int jo = 0, idx=0; jo < electron_list->size(); ++jo) {
    for (int ko = 0; ko < electron_list->size(); ++ko, ++ idx) {
      T_jo_ko[idx] = correlation_factor->f12o[idx] * traces.op12[idx]  * electron_list->inverse_weight[jo];
    }
  }
  calculate_bx_t_fb_3e_help(
    T_io_jo, T_io_ko, T_jo_ko,
    core_12o, correlation_factor->f12o_b,
    c_c4, electron_list->size());
  return (t[0] + t[1]);
}
...
double GF2_F12_VBX::calculate_bx_t_fc_3e(const Electron_Pair_List_Type* electron_pair_list, const Electron_List_Type* electron_list) {
  ...
  for(int ip = 0; ip < electron_pair_list->size();ip++) {
    for (int io = 0; io < electron_list->size(); ++io) {
      ...
      core_12p[ip]     += c4 * cf * traces.dp32[ip*traces.electrons+io] * traces.k13[ip * traces.electrons + io] * wgt;
      core_22p[ip]     -= c3 * cf * traces.dp31[ip*traces.electrons+io]  * traces.k13[ip * traces.electrons + io] * wgt;
...
double GF2_F12_VBX::calculate_bx_t_fc_4e(const Electron_Pair_List_Type* electron_pair_list, const Electron_List_Type* electron_list) {
  ...
  calculate_bx_t_fa_4e_help(T_ip_jo, traces.p23, traces.v23, T_ip_io, T_io_jo, core_d13, traces.v13, T_ip,  c_c3, electron_list->size(), electron_pair_list->size());
  calculate_bx_t_fa_4e_help(T_ip_jo, traces.p23, traces.k23, T_ip_io, T_io_jo, core_d13, traces.k13, T_ip, -c_c3, electron_list->size(), electron_pair_list->size());
  calculate_bx_t_fa_4e_help(T_ip_jo, traces.p23, traces.v13, T_ip_io, T_io_jo, core_d13, traces.v23, T_ip,  c_c4, electron_list->size(), electron_pair_list->size());
  calculate_bx_t_fa_4e_help(T_ip_jo, traces.p23, traces.k13, T_ip_io, T_io_jo, core_d13, traces.k23, T_ip, -c_c4, electron_list->size(), electron_pair_list->size());

  calculate_bx_t_fa_4e_help(T_ip_jo, traces.p13, traces.k13, T_ip_io, T_io_jo, core_d23, traces.k23, T_ip,  c_c3, electron_list->size(), electron_pair_list->size());
  calculate_bx_t_fa_4e_help(T_ip_jo, traces.p13, traces.v13, T_ip_io, T_io_jo, core_d23, traces.v23, T_ip, -c_c3, electron_list->size(), electron_pair_list->size());
  calculate_bx_t_fa_4e_help(T_ip_jo, traces.p13, traces.k23, T_ip_io, T_io_jo, core_d23, traces.k13, T_ip,  c_c4, electron_list->size(), electron_pair_list->size());
  calculate_bx_t_fa_4e_help(T_ip_jo, traces.p13, traces.v23, T_ip_io, T_io_jo, core_d23, traces.v13, T_ip, -c_c4, electron_list->size(), electron_pair_list->size());

  calculate_bx_t_fa_4e_help(T_ip_jo, traces.dp32, traces.k23, T_ip_io, T_io_jo, core_13, traces.k13, T_ip,  c_c3, electron_list->size(), electron_pair_list->size());
  calculate_bx_t_fa_4e_help(T_ip_jo, traces.dp32, traces.v23, T_ip_io, T_io_jo, core_13, traces.v13, T_ip, -c_c3, electron_list->size(), electron_pair_list->size());
  calculate_bx_t_fa_4e_help(T_ip_jo, traces.dp32, traces.k13, T_ip_io, T_io_jo, core_13, traces.k23, T_ip,  c_c4, electron_list->size(), electron_pair_list->size());
  calculate_bx_t_fa_4e_help(T_ip_jo, traces.dp32, traces.v13, T_ip_io, T_io_jo, core_13, traces.v23, T_ip, -c_c4, electron_list->size(), electron_pair_list->size());
                                                                                                   
  calculate_bx_t_fa_4e_help(T_ip_jo, traces.dp31, traces.v13, T_ip_io, T_io_jo, core_23, traces.v23, T_ip,  c_c3, electron_list->size(), electron_pair_list->size());
  calculate_bx_t_fa_4e_help(T_ip_jo, traces.dp31, traces.k13, T_ip_io, T_io_jo, core_23, traces.k23, T_ip, -c_c3, electron_list->size(), electron_pair_list->size());
  calculate_bx_t_fa_4e_help(T_ip_jo, traces.dp31, traces.v23, T_ip_io, T_io_jo, core_23, traces.v13, T_ip,  c_c4, electron_list->size(), electron_pair_list->size());
...
void calculate_bx_k_4e_help_2(
    std::vector<double>& T_io_jo, const std::vector<double>& S_ip_io, const std::vector<double>& S_ip_jo,
    std::vector<double>& out, const std::vector<double>& S_io_jo,
    const double alpha, size_t size, size_t size_ep
    ) {
...
double GF2_F12_VBX::calculate_bx_k_4e(const Electron_Pair_List_Type* electron_pair_list, const Electron_List_Type* electron_list) {
  ...
  std::transform(correlation_factor->f13.begin(), correlation_factor->f13.end(), correlation_factor->f23.begin(), T_ip_io.begin(), std::minus<>());

////////////////////////////////////////////////////////////////////////////////////////////////////

  for (int ip = 0, idx=0; ip < electron_pair_list->size();ip++) {
    for (int jo = 0; jo < electron_list->size(); ++jo, idx++) {
      T_ip_jo[idx] = traces.k13[ip * traces.electrons + jo] * T_ip[ip];
    }
  }
  for (int io = 0, idx=0; io < electron_list->size();io++) {
    for (int jo = 0; jo < electron_list->size(); ++jo, idx++) {
  ...
  for (int io = 0, idx=0; io < electron_list->size();io++) {
    for (int jo = 0; jo < electron_list->size(); ++jo, idx++) {
      T_io_lo[idx] = T_io_jo[idx] * traces.op11[jo];
    }
  }
  calculate_bx_k_4e_help_1(
    T_ip_ko, T_ip_io, T_io_lo,
    core_23, T_ip_jo,
    c_c3, electron_list->size(), electron_pair_list->size());

  std::transform(T_io_jo.begin(), T_io_jo.end(), traces.op12.begin(), T_io_lo.begin(), std::multiplies<>());
  calculate_bx_k_4e_help_1(
    T_ip_ko, T_ip_jo, T_io_lo,
    core_23, T_ip_io,
    c_c4, electron_list->size(), electron_pair_list->size());

  std::transform(T_ip_io.begin(), T_ip_io.end(), traces.p23.begin(), T_ip_ko.begin(), std::multiplies<>());
  calculate_bx_k_4e_help_2(
    T_io_ko, T_ip_ko, T_ip_jo,
    core_12o, T_io_jo,
    c_c4, electron_list->size(), electron_pair_list->size());

  std::transform(T_ip_jo.begin(), T_ip_jo.end(), traces.p23.begin(), T_ip_ko.begin(), std::multiplies<>());
  for (int ip = 0; ip < electron_pair_list->size();ip++) {
    for (int io = 0; io < electron_list->size(); ++io) {
      double t_io = 0.0;
      for (int jo = 0; jo < electron_list->size(); ++jo) {
        t_io += T_io_jo[io*traces.electrons + jo] * T_ip_ko[ip * traces.electrons + jo];
      }
      core_11o[io] += c_c3 * t_io * T_ip_io[ip*traces.electrons+io];
    }
  }
////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  for (int io = 0, idx=0; io < electron_list->size();io++) {
    for (int jo = 0; jo < electron_list->size(); ++jo, idx++) {
      T_io_jo[idx] = traces.ok12[io * traces.electrons + jo] * electron_list->inverse_weight[io] * electron_list->inverse_weight[jo];
    }
  }
  for (int ip = 0, idx=0; ip < electron_pair_list->size();ip++) {
    for (int jo = 0; jo < electron_list->size(); ++jo, idx++) {
      T_ip_jo[idx] = T_ip_io[ip*traces.electrons+jo] * traces.p23[ip * traces.electrons + jo] * T_ip[ip];
    }
  }
  calculate_bx_k_4e_help_1(
    T_ip_ko, T_ip_jo, T_io_jo,
    core_13, correlation_factor->f23,
    -c_c4, electron_list->size(), electron_pair_list->size());


  for (int ip = 0, idx=0; ip < electron_pair_list->size();ip++) {
    for (int jo = 0; jo < electron_list->size(); ++jo, idx++) {
      T_ip_jo[idx] = correlation_factor->f23[ip * traces.electrons + jo] * traces.p13[ip * traces.electrons + jo] * T_ip[ip];
    }
  }
  calculate_bx_k_4e_help_1(
    T_ip_ko, T_ip_jo, T_io_jo,
  ...
  for (int ip = 0, idx=0; ip < electron_pair_list->size();ip++) {
    for (int jo = 0; jo < electron_list->size(); ++jo, idx++) {
      T_ip_jo[idx] = correlation_factor->f23[ip * traces.electrons + jo] * traces.p12[ip] * T_ip[ip];
    }
  }
  calculate_bx_k_4e_help_2(
    T_io_ko, T_ip_io, T_ip_jo,
    core_12o, T_io_jo,
    -c_c3, electron_list->size(), electron_pair_list->size());

  std::transform(T_io_jo.begin(), T_io_jo.end(), traces.op12.begin(), T_io_jo.begin(), std::multiplies<>());
  for (int ip = 0; ip < electron_pair_list->size();ip++) {
    double t_ip = 0.0;
    for (int io = 0; io < electron_list->size(); ++io) {
      double t_io = 0.0;
      for (int jo = 0; jo < electron_list->size(); ++jo) {
        t_io += correlation_factor->f23[ip * traces.electrons + jo] * T_io_jo[io * traces.electrons + jo];
      }
      t_ip += t_io * T_ip_io[ip*traces.electrons+io];
    }
    core_12p[ip] -= c_c3 * t_ip * T_ip[ip];
  }
  return (t[0] + t[1]);
}
