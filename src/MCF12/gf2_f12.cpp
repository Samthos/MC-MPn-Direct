  ...
  for (int band = 0; band < numBand; band++) {
    ...
    for (int ip = 0; ip < electron_pair_list->size(); ++ip) {
      ...
      for (int io = 0; io < electron_list->size(); ++io) {
        t[0] += correlation_factor->f23[ip][io] * x_traces.x13[ip][io] * traces.k13[ip][io] / electron_list->weight[io];
        t[1] += correlation_factor->f23[ip][io] * traces.p13[ip][io] * traces.k13[ip][io] / electron_list->weight[io];

        t[2] += correlation_factor->f23[ip][io] * traces.p23[ip][io] * traces.k13[ip][io] / electron_list->weight[io];
        t[3] += correlation_factor->f23[ip][io] * x_traces.x23[ip][io] * traces.k13[ip][io] / electron_list->weight[io];
      }
...
void GF2_F12_VBX::calculate_bx_t_fa(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  ...
  for(int ip = 0; ip < electron_pair_list->size();ip++) {
    ...
    for (int io = 0; io < electron_list->size(); ++io) {
      ...
      t[2] += correlation_factor->f23[ip][io] * x_traces.x23[ip][io] * traces.k13[ip][io] / electron_list->weight[io];
      t[3] += correlation_factor->f23[ip][io] * traces.p23[ip][io] * traces.k13[ip][io] / electron_list->weight[io];
    }
    auto f_a = correlation_factor->f12p_a[ip];
  ...
  for(int ip = 0; ip < electron_pair_list->size();ip++) {
    ...
    for (int io = 0; io < electron_list->size(); ++io) {
      ...
      for(int jo = 0; jo < electron_list->size();++jo) {
        if (jo != io) {
          s[0] += correlation_factor->f12o[io][jo] * x_traces.x23[ip][jo] * traces.k23[ip][jo] / (electron_list->weight[jo]);
          s[1] += correlation_factor->f12o[io][jo] * traces.p23[ip][jo]   * traces.k23[ip][jo] / (electron_list->weight[jo]);
                                                                                  
          s[2] += correlation_factor->f12o[io][jo] * x_traces.x23[ip][jo] * traces.v23[ip][jo] / (electron_list->weight[jo]);
          s[3] += correlation_factor->f12o[io][jo] * traces.p23[ip][jo]   * traces.v23[ip][jo] / (electron_list->weight[jo]);

          s[4] += correlation_factor->f12o[io][jo] * x_traces.x13[ip][jo] * traces.k23[ip][jo] / (electron_list->weight[jo]);
          s[5] += correlation_factor->f12o[io][jo] * traces.p13[ip][jo]   * traces.k23[ip][jo] / (electron_list->weight[jo]);
             
          s[6] += correlation_factor->f12o[io][jo] * x_traces.x13[ip][jo] * traces.v23[ip][jo] / (electron_list->weight[jo]);
          s[7] += correlation_factor->f12o[io][jo] * traces.p13[ip][jo]   * traces.v23[ip][jo] / (electron_list->weight[jo]);
        }
      }
      t[0] += s[0] * traces.p13[ip][io]   * traces.k13[ip][io] / (electron_list->weight[io]);
      t[1] += s[1] * x_traces.x13[ip][io] * traces.k13[ip][io] / (electron_list->weight[io]);
                                                              
      t[2] += s[2] * traces.p13[ip][io]   * traces.v13[ip][io] / (electron_list->weight[io]);
      t[3] += s[3] * x_traces.x13[ip][io] * traces.v13[ip][io] / (electron_list->weight[io]);
                                                              
      t[4] += s[4] * traces.p23[ip][io]   * traces.k13[ip][io] / (electron_list->weight[io]);
      t[5] += s[5] * x_traces.x23[ip][io] * traces.k13[ip][io] / (electron_list->weight[io]);
                                                                              
      t[6] += s[6] * traces.p23[ip][io]   * traces.v13[ip][io] / (electron_list->weight[io]);
      t[7] += s[7] * x_traces.x23[ip][io] * traces.v13[ip][io] / (electron_list->weight[io]);
    }
    auto f_a = correlation_factor->f12p_a[ip];
...
double calculate_bx_t_fb_4e(
  ...
  for (int jo = 0; jo < weight.size(); ++jo) {
    for (int lo = 0; lo < weight.size(); ++lo) {
      T_jo_lo[jo][lo] = traces.op12[jo][lo] * traces.ok12[jo][lo];
    }
    T_jo_lo[jo][jo] = 0.0;
  }
  ...
  for (int io = 0; io < weight.size(); ++io) {
    ...
    for (int ko = 0; ko < weight.size(); ++ko) {
      if (ko != io) {
        ...
        t_io += t_ko * x_traces.ox12[io][ko] * traces.ok12[io][ko] / weight[ko];
      }
    }
    en += t_io / weight[io];
  }
  ...
  for (int io = 0; io < weight.size(); ++io) {
    ...
    for (int jo = 0; jo < weight.size(); ++jo) {
      if (jo != io) {
        ...
        for (int ko = 0; ko < weight.size(); ++ko) {
          if (ko != jo && ko != io) {
            t_jo += correlation_factor->f12o[ko][io] * x_traces.ox12[io][ko] * traces.ok12[io][ko] / weight[ko];
          }
        }
        auto f_b = correlation_factor->f12o_b[io][jo];
        t_io += t_jo * f_b * traces.op12[jo][io] * traces.ok12[jo][io] / weight[jo] / (weight[io]);
      }
    }
...
void GF2_F12_VBX::calculate_bx_t_fb(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  for (int io = 0; io < electron_list->size(); ++io) {
    ...
    for (int jo = 0; jo < electron_list->size(); ++jo) {
      if (jo != io) {
        ...
        t_io[2] += f_b * correlation_factor->f12o[io][jo] * x_traces.ox12[io][jo] * traces.op12[io][jo] / electron_list->weight[jo];
        t_io[3] += f_b * correlation_factor->f12o[io][jo] * traces.op12[io][jo] * x_traces.ox12[io][jo] / electron_list->weight[jo];
      }
    }
    ...
    for (int jo = 0; jo < electron_list->size(); ++jo) {
      if (jo != io) {
        ...
        for (int ko = 0; ko < electron_list->size(); ++ko) {
          if (ko != jo && ko != io) {
          t_jo[0] += correlation_factor->f12o[jo][ko] * x_traces.ox12[io][ko] * traces.ok12[io][ko] / electron_list->weight[ko];
          t_jo[1] += correlation_factor->f12o[jo][ko] * traces.op12[io][ko]   * traces.ok12[io][ko] / electron_list->weight[ko];
          t_jo[2] += correlation_factor->f12o[jo][ko] * x_traces.ox12[jo][ko] * traces.ok12[io][ko] / electron_list->weight[ko];
          t_jo[3] += correlation_factor->f12o[jo][ko] * traces.op12[jo][ko]   * traces.ok12[io][ko] / electron_list->weight[ko];
          }
        }
        auto f_b = correlation_factor->f12o_b[io][jo];
        t_io[0] += f_b * t_jo[0] * traces.op11[jo]       / electron_list->weight[jo];
        t_io[1] += f_b * t_jo[1] * x_traces.ox11[jo]     / electron_list->weight[jo];
        t_io[2] += f_b * t_jo[2] * traces.op12[io][jo]   / electron_list->weight[jo];
  ...
  t_ko[1] += correlation_factor->f12o[ko][lo] * x_traces.ox12[jo][lo] * traces.ok12[jo][lo] / (electron_list->weight[lo]);
 
  t_ko[2] += correlation_factor->f12o[ko][lo] * traces.op12[io][lo]   * traces.ok12[jo][lo] / (electron_list->weight[lo]);
  t_ko[3] += correlation_factor->f12o[ko][lo] * x_traces.ox12[io][lo] * traces.ok12[jo][lo] / (electron_list->weight[lo]);

  t_ko[4] += correlation_factor->f12o[ko][lo] * traces.op12[jo][lo]   * traces.ov12[jo][lo] / (electron_list->weight[lo]);
  t_ko[5] += correlation_factor->f12o[ko][lo] * x_traces.ox12[jo][lo] * traces.ov12[jo][lo] / (electron_list->weight[lo]);

  t_ko[6] += correlation_factor->f12o[ko][lo] * traces.op12[io][lo]   * traces.ov12[jo][lo] / (electron_list->weight[lo]);
  t_ko[7] += correlation_factor->f12o[ko][lo] * x_traces.ox12[io][lo] * traces.ov12[jo][lo] / (electron_list->weight[lo]);
              }
            }
  auto f_b = correlation_factor->f12o_b[io][jo];
  t_jo[0] += t_ko[0] * x_traces.ox12[io][ko] * traces.ok12[io][ko] / electron_list->weight[ko];
  t_jo[1] += t_ko[1] * traces.op12[io][ko]   * traces.ok12[io][ko] / electron_list->weight[ko];
         
  t_jo[2] += t_ko[2] * x_traces.ox12[jo][ko] * traces.ok12[io][ko] / electron_list->weight[ko];
  t_jo[3] += t_ko[3] * traces.op12[jo][ko]   * traces.ok12[io][ko] / electron_list->weight[ko];
         
  t_jo[4] += t_ko[4] * x_traces.ox12[io][ko] * traces.ov12[io][ko] / electron_list->weight[ko];
  t_jo[5] += t_ko[5] * traces.op12[io][ko]   * traces.ov12[io][ko] / electron_list->weight[ko];
         
  t_jo[6] += t_ko[6] * x_traces.ox12[jo][ko] * traces.ov12[io][ko] / electron_list->weight[ko];
  t_jo[7] += t_ko[7] * traces.op12[jo][ko]   * traces.ov12[io][ko] / electron_list->weight[ko];
          }
        }
  auto f_b = correlation_factor->f12o_b[io][jo];
...
void GF2_F12_VBX::calculate_bx_k(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  for (int ip = 0; ip < electron_pair_list->size();ip++) {
    ...
    for (int io = 0; io < electron_list->size(); ++io) {
     ...
     xchang_1_pair_1_one_ints[2] += correlation_factor->f23[ip][io] * (correlation_factor->f23[ip][io] - correlation_factor->f13[ip][io]) * x_traces.x13[ip][io] * traces.p23[ip][io] * traces.k12[ip] * electron_pair_list->rv[ip] / electron_list->weight[io];
     xchang_1_pair_1_one_ints[2] += correlation_factor->f23[ip][io] * (correlation_factor->f23[ip][io] - correlation_factor->f13[ip][io]) * traces.p13[ip][io] * x_traces.x23[ip][io] * traces.k12[ip] * electron_pair_list->rv[ip] / electron_list->weight[io];
    }
  }
...
direct_1_pair_2_one_ints[5] += f24p * (f23 - f13) * x_traces.ox12[io][jo] * traces.p12[ip]      * traces.ok12[io][jo] * traces.k12[ip] * wgt;
direct_1_pair_2_one_ints[5] += f24p * (f23 - f13) * traces.op12[io][jo] * x_traces.x12[ip]      * traces.ok12[io][jo] * traces.k12[ip] * wgt;

xchang_1_pair_2_one_ints[5] += f24p * (f23 - f13) * x_traces.x13[ip][jo]  * traces.p23[ip][io]  * traces.ok12[io][jo] * traces.k12[ip] * wgt;
xchang_1_pair_2_one_ints[5] += f24p * (f23 - f13) * traces.p13[ip][jo]  * x_traces.x23[ip][io]  * traces.ok12[io][jo] * traces.k12[ip] * wgt;
        }
      }
    }
  }
