...
double MP2_F12_V::calculate_v_2e(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
...
double MP2_F12_V::calculate_v_3e(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  std::array<double, 2> t{0.0, 0.0};
  for (int ip = 0; ip < electron_pair_list->size(); ++ip) {
...
double MP2_F12_V::calculate_v_4e(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  std::array<double, 2> t{0.0, 0.0};
  for (int ip = 0; ip < electron_pair_list->size(); ++ip) {
    ...
    for (int io = 0; io < electron_list->size(); ++io) {
      ...
      for (int jo = 0; jo < electron_list->size(); ++jo) {
        t_io[0] += correlation_factor->f12o[io * traces.electrons + jo] * traces.p23[ip * traces.electrons + jo] * traces.k23[ip * traces.electrons + jo] * electron_list->inverse_weight[jo];
        t_io[1] += correlation_factor->f12o[io * traces.electrons + jo] * traces.p13[ip * traces.electrons + jo] * traces.k23[ip * traces.electrons + jo] * electron_list->inverse_weight[jo];
        t_io[2] += correlation_factor->f12o[io * traces.electrons + jo] * traces.p23[ip * traces.electrons + jo] * traces.v23[ip * traces.electrons + jo] * electron_list->inverse_weight[jo];
        t_io[3] += correlation_factor->f12o[io * traces.electrons + jo] * traces.p13[ip * traces.electrons + jo] * traces.v23[ip * traces.electrons + jo] * electron_list->inverse_weight[jo];
      }
      t_ip[0] += traces.p13[ip * traces.electrons + io] * (t_io[0] * traces.k13[ip * traces.electrons + io] - t_io[2] * traces.v13[ip * traces.electrons + io]) * electron_list->inverse_weight[io];
      t_ip[1] += traces.p23[ip * traces.electrons + io] * (t_io[1] * traces.k13[ip * traces.electrons + io] - t_io[3] * traces.v13[ip * traces.electrons + io]) * electron_list->inverse_weight[io];
    }
    t[0] += t_ip[0] * electron_pair_list->rv[ip];
    t[1] += t_ip[1] * electron_pair_list->rv[ip];
  }
...
double MP2_F12_VBX::calculate_bx_t_fa_2e(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  std::array<double, 2> t{0.0, 0.0};
  for(int ip = 0; ip < electron_pair_list->size(); ip++) {
    auto f_a = correlation_factor->f12p_a[ip];
    auto f_12 = correlation_factor->f12p[ip];
    t[0] += f_12 * f_a * traces.p11[ip] * traces.p22[ip] * electron_pair_list->rv[ip];
    t[1] += f_12 * f_a * traces.p12[ip] * traces.p12[ip] * electron_pair_list->rv[ip];
  }
  return (c3 * t[0] + c4 * t[1]) * nsamp_pair;
}
double MP2_F12_VBX::calculate_bx_t_fa_3e(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  std::array<double, 2> t{0.0, 0.0};
  for(int ip = 0; ip < electron_pair_list->size(); ip++) {
    std::array<double, 2> t_ip{0.0, 0.0};
    for (int io = 0; io < electron_list->size(); ++io) {
    ...
    t[1] += t_ip[1] * f_a * traces.p12[ip] * electron_pair_list->rv[ip];
  }
  return -2.0 * (c3 * t[0] + c4 * t[1]) * nsamp_pair * nsamp_one_1;
}
double MP2_F12_VBX::calculate_bx_t_fa_4e(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  std::array<double, 2> t{0.0, 0.0};
  for(int ip = 0; ip < electron_pair_list->size(); ip++) {
    std::array<double, 4> t_ip{0.0, 0.0, 0.0, 0.0};
    for (int io = 0; io < electron_list->size(); ++io) {
      ...
      for(int jo = 0; jo < electron_list->size();++jo) {
        t_io[0] += correlation_factor->f12o[io * traces.electrons + jo] * traces.p23[ip * traces.electrons + jo] * traces.k23[ip * traces.electrons + jo] * electron_list->inverse_weight[jo];
        t_io[1] += correlation_factor->f12o[io * traces.electrons + jo] * traces.p23[ip * traces.electrons + jo] * traces.v23[ip * traces.electrons + jo] * electron_list->inverse_weight[jo];
        t_io[2] += correlation_factor->f12o[io * traces.electrons + jo] * traces.p13[ip * traces.electrons + jo] * traces.k23[ip * traces.electrons + jo] * electron_list->inverse_weight[jo];
        t_io[3] += correlation_factor->f12o[io * traces.electrons + jo] * traces.p13[ip * traces.electrons + jo] * traces.v23[ip * traces.electrons + jo] * electron_list->inverse_weight[jo];
      }
      t_ip[0] += t_io[0] * traces.p13[ip * traces.electrons + io] * traces.k13[ip * traces.electrons + io] * electron_list->inverse_weight[io];
      t_ip[1] += t_io[1] * traces.p13[ip * traces.electrons + io] * traces.v13[ip * traces.electrons + io] * electron_list->inverse_weight[io];
      t_ip[2] += t_io[2] * traces.p23[ip * traces.electrons + io] * traces.k13[ip * traces.electrons + io] * electron_list->inverse_weight[io];
      t_ip[3] += t_io[3] * traces.p23[ip * traces.electrons + io] * traces.v13[ip * traces.electrons + io] * electron_list->inverse_weight[io];
    }
    auto f_a = correlation_factor->f12p_a[ip];
    t[0] += t_ip[0] * f_a * electron_pair_list->rv[ip];
    t[0] -= t_ip[1] * f_a * electron_pair_list->rv[ip];
    t[1] += t_ip[2] * f_a * electron_pair_list->rv[ip];
    t[1] -= t_ip[3] * f_a * electron_pair_list->rv[ip];
  }
  return (c3 * t[0] + c4 * t[1]) * nsamp_pair * nsamp_one_2;
}
double MP2_F12_VBX::calculate_bx_t_fa(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  double en = 0.0;
...
double MP2_F12_VBX::calculate_bx_t_fb_2e(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  std::array<double, 2> t{0.0, 0.0};
  for (int io = 0; io < electron_list->size(); ++io) {
...
double MP2_F12_VBX::calculate_bx_t_fb_3e(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  std::array<double, 2> t{0.0, 0.0};
  for (int io = 0; io < electron_list->size(); ++io) {
    std::array<double, 2> t_io{0.0, 0.0};
    for (int jo = 0; jo < electron_list->size(); ++jo) {
...
double MP2_F12_VBX::calculate_bx_t_fb_4e(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  std::array<double, 2> t{0.0, 0.0};
...
double MP2_F12_VBX::calculate_bx_t_fb(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  double en = 0.0;
...
double MP2_F12_VBX::calculate_bx_t_fc_2e(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  std::array<double, 2> t{0.0, 0.0};
  for(int ip = 0; ip < electron_pair_list->size(); ip++) {
    auto f_12 = correlation_factor->f12p[ip];
...
double MP2_F12_VBX::calculate_bx_t_fc_3e(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  std::array<double, 2> t{0.0, 0.0};
  for(int ip = 0; ip < electron_pair_list->size(); ip++) {
    std::array<double, 4> t_ip{0.0, 0.0, 0.0, 0.0};
    for (int io = 0; io < electron_list->size(); ++io) {
      t_ip[0] += correlation_factor->f23[ip * traces.electrons + io] *  traces.p13[ip * traces.electrons + io] * traces.k13[ip * traces.electrons + io] * electron_list->inverse_weight[io];
      t_ip[1] += correlation_factor->f23[ip * traces.electrons + io] * traces.dp31[ip][io] * traces.k13[ip * traces.electrons + io] * electron_list->inverse_weight[io];

      t_ip[2] += correlation_factor->f23[ip * traces.electrons + io] * traces.dp32[ip][io] * traces.k13[ip * traces.electrons + io] * electron_list->inverse_weight[io];
...
double MP2_F12_VBX::calculate_bx_t_fc_4e(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  std::array<double, 2> t{0.0, 0.0};
  for (int ip = 0; ip < electron_pair_list->size(); ip++) {
    std::array<double, 4> t_ip{0.0, 0.0, 0.0, 0.0};
    for (int io = 0; io < electron_list->size(); ++io) {
      ...
      for (int jo = 0; jo < electron_list->size(); ++jo) {
          t_io[0] += correlation_factor->f12o[io * traces.electrons + jo] * traces.dp32[ip][jo]                     * traces.k23[ip * traces.electrons + jo] * electron_list->inverse_weight[jo];
          ...
          t_io[2] += correlation_factor->f12o[io * traces.electrons + jo] * traces.dp32[ip][jo]                     * traces.v23[ip * traces.electrons + jo] * electron_list->inverse_weight[jo];
          ...
          t_io[4] += correlation_factor->f12o[io * traces.electrons + jo] *  traces.p13[ip * traces.electrons + jo] * traces.k23[ip * traces.electrons + jo] * electron_list->inverse_weight[jo];
          t_io[5] += correlation_factor->f12o[io * traces.electrons + jo] * traces.dp31[ip][jo]                     * traces.k23[ip * traces.electrons + jo] * electron_list->inverse_weight[jo];

          t_io[6] += correlation_factor->f12o[io * traces.electrons + jo] *  traces.p13[ip * traces.electrons + jo] * traces.v23[ip * traces.electrons + jo] * electron_list->inverse_weight[jo];
          t_io[7] += correlation_factor->f12o[io * traces.electrons + jo] * traces.dp31[ip][jo]                     * traces.v23[ip * traces.electrons + jo] * electron_list->inverse_weight[jo];
      }
      t_ip[0] += (t_io[0] *  traces.p13[ip * traces.electrons + io] - t_io[1] * traces.dp31[ip][io]) * traces.k13[ip * traces.electrons + io] * electron_list->inverse_weight[io];
      t_ip[1] += (t_io[2] *  traces.p13[ip * traces.electrons + io] - t_io[3] * traces.dp31[ip][io]) * traces.v13[ip * traces.electrons + io] * electron_list->inverse_weight[io];
      t_ip[2] += (t_io[4] * traces.dp32[ip][io] - t_io[5] *  traces.p23[ip * traces.electrons + io]) * traces.k13[ip * traces.electrons + io] * electron_list->inverse_weight[io];
      t_ip[3] += (t_io[6] * traces.dp32[ip][io] - t_io[7] *  traces.p23[ip * traces.electrons + io]) * traces.v13[ip * traces.electrons + io] * electron_list->inverse_weight[io];
    }
    auto f_c = correlation_factor->f12p_c[ip];
    t[0] += t_ip[0] * f_c * electron_pair_list->rv[ip];
    t[0] -= t_ip[1] * f_c * electron_pair_list->rv[ip];
    t[1] += t_ip[2] * f_c * electron_pair_list->rv[ip];
    t[1] -= t_ip[3] * f_c * electron_pair_list->rv[ip];
  }
  return (c3 * t[0] + c4 * t[1]) * nsamp_pair * nsamp_one_2;
}
double MP2_F12_VBX::calculate_bx_t_fc(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  double en = 0.0;
...
double MP2_F12_VBX::calculate_bx_t_fd_2e(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  std::array<double, 2> t{0.0, 0.0};
  for (int io = 0; io < electron_list->size(); ++io) {
    std::array<double, 2> t_io{0.0, 0.0};
    for (int jo = 0; jo < electron_list->size(); ++jo) {
...
double MP2_F12_VBX::calculate_bx_t_fd_3e(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  std::array<double, 2> t{0.0, 0.0};
  for (int io = 0; io < electron_list->size(); ++io) {
...
double MP2_F12_VBX::calculate_bx_t_fd_4e(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  std::array<double, 2> t{0.0, 0.0};
  for (int io = 0; io < electron_list->size(); ++io) {
    std::array<double, 4> t_io{0.0, 0.0, 0.0, 0.0};
    for (int jo = 0; jo < electron_list->size(); ++jo) {
...
double MP2_F12_VBX::calculate_bx_t_fd(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  double en = 0.0;
...
double MP2_F12_VBX::calculate_bx_k_3e(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  std::array<double, 2> t{0.0, 0.0};
  for (int ip = 0; ip < electron_pair_list->size(); ip++) {
    std::array<double, 2> t_io{0.0, 0.0};
    for (int io = 0; io < electron_list->size(); ++io) {
...
double MP2_F12_VBX::calculate_bx_k_4e(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  std::array<double, 2> t{0.0, 0.0};
...
double MP2_F12_VBX::calculate_bx_k_5e(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  std::array<double, 2> t{0.0, 0.0};
...
double MP2_F12_VBX::calculate_bx_k(const Electron_Pair_List* electron_pair_list, const Electron_List* electron_list) {
  double en = 0.0;
